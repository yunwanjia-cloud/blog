...
$("#btnfold").click(function() {
	if (fold) {
		...
		$("#player",parent.document).animate({
			width:top.player.jp_container_N.scrollWidth - 480
		}, 'slow');
	} else {
		$(".jp-video").animate({
			left: 0
		...
		$("#player",parent.document).animate({
			width:top.player.jp_container_N.scrollWidth
		});
	}
});
...
$("#listClose").click(function() {
	if (iOpen) {
		...
		$("#player",parent.document).animate({
			height: $("#player",parent.document).height() - 80
		});
		$("#btnfold").css({
			top: "5px"
		});
